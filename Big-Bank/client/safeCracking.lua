...
AddEventHandler("pacific_safecracking:loop", function(id)
		...
		while cracking do
			...
			DisplayHelpText("[E] Crack the safe")
			...
			if GetDistanceBetweenCoords(mybasepos, GetEntityCoords(GetPlayerPed(-1))) > 1 or curLock > difficulty then
				cracking = false
				safeboxing = false
				if(GetDistanceBetweenCoords(mybasepos, GetEntityCoords(GetPlayerPed(-1))) > 1)then
					TriggerServerEvent("pacificBankRobbery:server:busyState", "safe", id, false)
				end
		...
		if curLock > difficulty then
			resetAnim()
			takeAnim()
			TriggerServerEvent("pacificBankRobbery:server:stealableOpen", GetPlayerServerId(globalPlayerId), "safe", id, serverKeyGen)
		end
		Citizen.Wait(1000)
		TriggerServerEvent("pacificBankRobbery:server:busyState", "safe", id, false)
		safeboxing = false
    ...
    local player = GetPlayerPed( -1 )
  	if ( DoesEntityExist( player ) and not IsEntityDead( player )) then
        ...
        if animType == 3 then
			...
			print("You get nothing! You dickhead!")
	    end
